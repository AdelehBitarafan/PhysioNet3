...
def calc_weights(labels, weights, identity_w=0.7):
...
def prepare_data(x, length, mod=0, scale=1000., clip1=-100., clip2=100., aug=True,
        p_art1=0.03, p_art2=0.03, p_noise1=0.02, p_noise2=0.03, p_filt1=0.02, p_filt2=0.03,
        p_shfl=0.03, p_chzero1=0.03, p_chzero2=0.03, p_inv1=0.02, p_inv2=0.02, p_inv3=0.03,
        p_scale=0.08, p_scale2=0.05, p_shft=0.07):
...
def Add_rpeak_channel(data):
    ...
    for i in range(len(data)):
        r_peak = select_rpeaks(data[i])
        r_peak = r_peak[..., np.newaxis]
        new_data[i] = np.concatenate((data[i],r_peak),axis=1)
    return new_data

...
def branch(inputs, filter1, conv_size1):
    x = tf.keras.layers.Conv1D(filter1, conv_size1, 4, padding='SAME')(inputs)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Dropout(rate=0.1)(x)
    x = tf.keras.layers.Activation('relu')(x)
    #x = tf.keras.layers.MaxPooling1D((2))(x)
...
def create_model_resNet(inputs):
    ...
    x_right = branch(inputs, filter1=128, conv_size1=(15))

    x = tf.keras.layers.add([x_left, x_right])

    x = tf.keras.layers.Conv1D(64, (3), 2, padding='SAME')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.Dropout(rate=0.1)(x)
    x = tf.keras.layers.Activation('relu')(x)
    #x = tf.keras.layers.MaxPooling1D((2))(x)
    ...
    e5 = tf.reshape(x, [-1, e5_shape[-2]*e5_shape[-1]])

    return e5


def create_model_hasani(X):
    
    e1 = tf.layers.conv1d(X, 48, 19, strides=(4), padding='valid', activation=tf.nn.relu, use_bias=True)
    e2 = tf.layers.conv1d(e1, 80, (15), strides=(3), activation=tf.nn.relu, use_bias=True)
    e3 = tf.layers.max_pooling1d(tf.layers.conv1d(e2, 112, (9), strides=(2), activation=tf.nn.relu, use_bias=True), 2,2)
    e4 = tf.layers.max_pooling1d(tf.layers.conv1d(e3, 144, (9), strides=(1), activation=tf.nn.relu, use_bias=True),2,2)
    e5 = tf.layers.max_pooling1d(tf.layers.conv1d(e4, 160, (7), strides=(1), activation=tf.nn.relu, use_bias=True), 2,2)
    
    print(e5.shape)
    
    e5_shape = e5.shape.as_list()
    e5_rs = tf.reshape(e5, [-1, e5_shape[-2]*e5_shape[-1]])
    ...
    while epoch < epoch_size:
        ...
        for b in range(n_b):
            ...
            feed = {X: train_batch, Y: batch_label, Y_W: batch_weight, KEEP_PROB:0.5,
                    ...
                    Gamma: gamma , learning_rate_placeholder: learning_rate}
